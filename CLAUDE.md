...
├── schema_v1.sql               # **DB 真理源**:9 表 + 4 视图 + INSERT 8 家
...
│   ├── components/
│   │   ├── LocaleSwitcher.tsx  # 原生 select dropdown,layout 全局右上角
│   │   └── LocalDateTime.tsx   # 客户端组件,SSR fallback + 浏览器本地时区 toLocaleString
...
- 视图 (`v_leaderboard` / `v_today_matches` / `v_upcoming_matches` / `v_model_reliability`) 在 Prisma 里是只读类型,用 `@unique` 不能用 `@id`
- 视图加列必须 `DROP VIEW + CREATE VIEW`(Postgres `CREATE OR REPLACE VIEW` 不允许改列序),例如 `v_upcoming_matches` 加 `home_name_en` 时就踩过
...
- **AI 生成的 `reason` / `wildcard` 走双语方案**(prompt v1.1 起):同一个 prompt 让 AI 同时输出 `reason_zh` + `reason_en` + `wildcard_zh` + `wildcard_en`,落库到 `predictions_l1.reason` (zh) / `reason_en` / `wildcard` (zh) / `wildcard_en`。前端按 locale 取值,`reason_en` 为 null 时 fallback 到 `reason`(兼容 prompt v1.0 时代的旧数据)。**不要**额外做后台翻译或 en 单独跑一遍 — 同 prompt 同步出才保「8 家公平」
- 用 `src/i18n/navigation.ts` 导出的 `Link` / `useRouter`(自动带 locale 前缀),**不要**用 `next/link`
- 球队名取值用 `src/lib/team-name.ts` 的 `teamName(team, locale)`;视图返回的 `home_name` / `home_name_en` 双列在 page 里按 locale 取
- `LocaleSwitcher` 在 `src/app/[locale]/layout.tsx` 全局挂载(顶部右侧,内联非 fixed),所有页面自动有
- 「积分榜」用语统一改成「AI 排行榜」/「AI Leaderboard」 — 避免和球队积分混淆
...
- ✅ **解析必须容错**(实现在 `src/lib/openrouter.ts` 的 `parseJsonContent`):
  ...
  - 提取**第一个 `{` 到最后一个 `}`** 之间的内容
- ✅ **字段名映射**(`normalizeData`):`result ↔ outcome`、`home_score ↔ score_home`、`both_teams_to_score ↔ btts`、`reasoning/explanation ↔ reason`、`risk/wild_card ↔ wildcard`、`reason_zh ↔ reason`、`wildcard_zh ↔ wildcard`,比分字符串 `"2-1"` 自动 split
...
- **前端展示用浏览器本地时区**:`<LocalDateTime>` 客户端组件 SSR 渲一份 fallback(服务器时区 / next-intl),mount 后 `toLocaleString()` 用浏览器 TZ 覆盖,`suppressHydrationWarning` 抑制错位告警。新加日期字段都走这个组件
...
- `predictions_l1.reason` / `wildcard` 始终是中文(prompt 锁定 zh);`reason_en` / `wildcard_en` 是 v1.1 起 AI 同步输出的英文版(v1.0 旧数据为 null)
...
- `v_upcoming_matches`(首页主查询):`kickoff_at > now() AND status <> 'finished'`,同时返回 `home_name`(zh)/ `home_name_en` 双列
